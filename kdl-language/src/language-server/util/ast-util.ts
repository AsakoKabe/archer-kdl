...
import { AttributeMappingSourceType } from '@kdl/protocol';
...
import { ID_PROPERTY, IdProvider } from '../kdl-naming.js';
import { getLocalName } from '../kdl-scope.js';
...
export function createEntity(container: KDLRoot, id: string, opts?: Partial<Omit<Entity, '$container' | '$type' | 'id'>>): Entity {
    return {
        $container: container,
        $type: 'Entity',
        id,
...
export function findSemanticRoot<T extends SemanticRoot>(input: DocumentContent, guard?: TypeGuard<T>): SemanticRoot | T | undefined {
    ...
    const semanticRoot = isKDLRoot(root) ? (root.entity ?? root.mapping ?? root.relationship ?? root.systemDiagram) : undefined;
