...
import { AstKDLDocument } from '../../model-server/open-text-document-manager.js';
...
export class CrossModelStorage implements SourceModelStorage, ClientSessionListener {
   ...
   async loadSourceModel(action: RequestModelAction): Promise<void> {
      ...
      const document = await this.update(sourceUri);
      if (!document) {
         return;
      }
      this.toDispose.push(await this.state.modelService.open({ uri: sourceUri, clientId: this.state.clientId }));
      this.toDispose.push(
         this.state.modelService.onModelUpdated(sourceUri, async event => {
            if (this.state.clientId !== event.sourceClientId || event.reason !== 'changed') {
               const result = await this.updateAndSubmit(sourceUri, event.document);
   ...
   protected async update(uri: string, document?: AstKDLDocument): Promise<AstKDLDocument | undefined> {
   ...
   protected async updateEditMode(document: AstKDLDocument): Promise<Action[]> {
      const actions = [];
   ...
   protected updateAndSubmit = debounce(async (rootUri: string, document: AstKDLDocument): Promise<Action[]> => {
   ...
   saveSourceModel(action: SaveModelAction): MaybePromise<void> {
      ...
      AstUtils.streamReferences(this.state.semanticRoot)
         ...
         .nonNullable()
         .map(ref => AstUtils.findRootNode(ref) as KDLRoot)
         .forEach(root =>
