...
import { IMPLICIT_ID_PROPERTY } from '../language/util/ast-util.js';
...
export class ModelServer implements Disposable {
   ...
   protected async resolve(args: CrossReference): Promise<ResolvedElement | undefined> {
      ...
      const model = this.toSerializable(AstUtils.findRootNode(node)) as KDLRoot;
      return { uri, model };
   }

   protected async openModel(args: OpenModelArgs): Promise<KDLDocument | undefined> {
   ...
   protected async requestModel(uri: string): Promise<KDLDocument | undefined> {
   ...
   protected toDocument<T extends KDLDocument<ast.KDLRoot, Diagnostic>>(
      document: T
   ): KDLDocument<KDLRoot, ModelDiagnostic> {
      return {
         uri: document.uri,
    ...
    */
   protected toSerializable<T extends AstNode = ast.KDLRoot, O extends object = KDLRoot>(obj?: T): O | undefined {
      if (!obj) {
         return;
      }
