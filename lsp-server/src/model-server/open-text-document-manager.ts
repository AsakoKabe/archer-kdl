...
import { CloseModelArgs, KDLDocument, ModelSavedEvent, ModelUpdatedEvent, OpenModelArgs } from '@kdl/protocol';
...
import { KDLLanguageMetaData } from '../language/generated/module.js';
...
export type AstKDLDocument = KDLDocument<KDLRoot, Diagnostic>;
...
export class OpenTextDocumentManager {
    ...
    */
   onSave(uri: string, listener: (model: ModelSavedEvent<AstKDLDocument>) => void): Disposable {
      return this.textDocuments.onDidSave(async event => {
         ...
         if (event.document.uri === uri && documentURI !== undefined && this.langiumDocs.hasDocument(documentURI)) {
            ...
            const root = document.parseResult.value as KDLRoot;
            return listener({
               document: {
                  root,
   ...
   onUpdate(uri: string, listener: (model: ModelUpdatedEvent<AstKDLDocument>) => void): Disposable {
      return this.documentBuilder.onBuildPhase(DocumentState.Validated, (allChangedDocuments, _token) => {
         ...
         if (changedDocument) {
            ...
            const event: ModelUpdatedEvent<AstKDLDocument> = {
               document: {
                  root: changedDocument.parseResult.value as KDLRoot,
   ...
   protected createDummyDocument(uri: string): TextDocumentItem {
      return TextDocumentItem.create(this.normalizedUri(uri), KDLLanguageMetaData.languageId, 0, '');
   }

   protected async createDocumentFromFileSystem(
      uri: string,
      languageId: string = KDLLanguageMetaData.languageId
