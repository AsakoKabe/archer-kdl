...
import { KDLRoot, isKDLRoot } from '../language/generated/ast.js';
import { KDLServices, KDLSharedServices } from '../language/kdl-module.js';
import { PACKAGE_JSON } from '../language/kdl-package-manager.js';
import { findDocument } from '../language/util/ast-util.js';
import { AstKDLDocument } from './open-text-document-manager.js';
...
export class ModelService {
    constructor(
        protected shared: KDLSharedServices,
     ...
     * @param state minimum state the document should have before returning
     */
    async request(uri: string, state = DocumentState.Validated): Promise<AstKDLDocument | undefined> {
        ...
        return isKDLRoot(root) ? { root, diagnostics: document.diagnostics ?? [], uri } : undefined;
    }
     ...
     */
    async update(args: UpdateModelArgs<KDLRoot>): Promise<AstKDLDocument> {
        ...
        if (text === textDocument.getText()) {
            return {
                ...
                root: document.parseResult.value as KDLRoot,
                uri: args.uri
            };
        }
        const newVersion = textDocument.version + 1;
        const pendingUpdate = new Deferred<AstKDLDocument>();
        const listener = this.documentBuilder.onBuildPhase(DocumentState.Validated, (allChangedDocuments, _token) => {
            ...
            if (updatedDocument) {
                pendingUpdate.resolve({
                    ...
                    root: updatedDocument.parseResult.value as KDLRoot,
                    uri: args.uri
                });
                listener.dispose();
            }
        });
        const timeout = new Promise<AstKDLDocument>((_, reject) =>
            setTimeout(() => {
                listener.dispose();
    ...
    onModelUpdated(uri: string, listener: (model: ModelUpdatedEvent<AstKDLDocument>) => void): Disposable {
        return this.documentManager.onUpdate(uri, listener);
    }

    onModelSaved(uri: string, listener: (model: ModelSavedEvent<AstKDLDocument>) => void): Disposable {
     ...
     */
    async save(args: SaveModelArgs<KDLRoot>): Promise<void> {
    ...
    getId(node: AstNode, uri = findDocument(node)?.uri): string | undefined {
        if (uri) {
            const services = this.shared.ServiceRegistry.getServices(uri) as KDLServices;
    ...
    async findReferenceableElements(args: CrossReferenceContext): Promise<ReferenceableElement[]> {
        return this.shared.ServiceRegistry.KDL.references.ScopeProvider.complete(args);
    }

    async resolveCrossReference(args: CrossReference): Promise<AstNode | undefined> {
        return this.shared.ServiceRegistry.KDL.references.ScopeProvider.resolveCrossReference(args);
    }
